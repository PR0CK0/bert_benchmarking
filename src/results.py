...
class ResultsManager:
    ...
    def create_comparison_plots(
        ...
        # Plot 1: Latency comparison (average across batch sizes)
        fig, ax = plt.subplots()
        latency_data = df.groupby(['model_name', 'device'])['mean_latency_ms'].mean().reset_index()

        sns.barplot(data=latency_data, x='model_name', y='mean_latency_ms', hue='device', ax=ax)
        ax.set_xlabel('Model')
        ax.set_ylabel('Mean Latency (ms)')
        ax.set_title('Average Inference Latency by Model')
        ...
        sns.barplot(data=throughput_data, x='model_name', y='throughput_samples_per_sec', hue='device', ax=ax)
        ax.set_xlabel('Model')
        ax.set_ylabel('Throughput (samples/sec)')
        ax.set_title('Average Throughput by Model')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        ...
        sns.barplot(data=memory_data, x='model_name', y='peak_memory_mb', hue='device', ax=ax)
        ax.set_xlabel('Model')
        ax.set_ylabel('Peak Memory (MB)')
        ax.set_title('Average Peak Memory by Model')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        ...
        # Plot 4: Model size vs latency scatter
        fig, ax = plt.subplots()
        ...
        colors = {'cpu': '#1f77b4', 'cuda': '#ff7f0e'}

        # Plot points and collect text objects for adjustment
        texts = []
        for device in scatter_data['device'].unique():
            device_data = scatter_data[scatter_data['device'] == device]
            color = colors.get(device, '#2ca02c')

            # Plot scatter points
            ax.scatter(device_data['model_size_mb'], device_data['mean_latency_ms'],
                      label=device, s=100, alpha=0.6, color=color)

            # Add model name labels
            for idx, row in device_data.iterrows():
                # Annotate with model name
                text = ax.annotate(row['model_name'],
            ...
            # Add trend line for each device
            if len(device_data) > 1:
                z = np.polyfit(device_data['model_size_mb'], device_data['mean_latency_ms'], 1)
                p = np.poly1d(z)
                x_trend = np.linspace(device_data['model_size_mb'].min(),
                                     device_data['model_size_mb'].max(), 100)
                ax.plot(x_trend, p(x_trend), '--', color=color, alpha=0.3, linewidth=2)

        ax.set_xlabel('Model Size (MB)')
        ax.set_ylabel('Mean Latency (ms)')
        ax.set_title('Model Size vs Inference Latency')
        ax.legend()
        plt.tight_layout()
    ...
    def create_top_performers_plots(
        ...
        # Panel 1: Fastest (top 3)
        top_fast_3 = self.get_best_models(results, criterion="latency", top_k=3)
        top_fast_3_sorted = top_fast_3.sort_values('mean_latency_ms', ascending=False)
        colors1 = plt.cm.viridis(np.linspace(0.3, 0.9, len(top_fast_3_sorted)))
        axes[0].barh(range(len(top_fast_3_sorted)), top_fast_3_sorted['mean_latency_ms'],
                     color=colors1, alpha=0.8)
        axes[0].set_yticks(range(len(top_fast_3_sorted)))
        axes[0].set_yticklabels(top_fast_3_sorted.index, fontsize=9)
        axes[0].set_xlabel('Latency (ms)', fontsize=10)
        axes[0].set_title('Fastest Inference', fontsize=11, fontweight='bold')
        for i, (idx, row) in enumerate(top_fast_3_sorted.iterrows()):
            axes[0].text(row['mean_latency_ms'] + 0.5, i, f"{row['mean_latency_ms']:.1f}ms",
                        va='center', fontsize=8)

        # Panel 2: Memory Efficient (top 3)
        top_mem_3 = self.get_best_models(results, criterion="memory", top_k=3)
        top_mem_3_sorted = top_mem_3.sort_values('peak_memory_mb', ascending=False)
        colors2 = plt.cm.coolwarm_r(np.linspace(0.2, 0.8, len(top_mem_3_sorted)))
        axes[1].barh(range(len(top_mem_3_sorted)), top_mem_3_sorted['peak_memory_mb'],
                     color=colors2, alpha=0.8)
        axes[1].set_yticks(range(len(top_mem_3_sorted)))
        axes[1].set_yticklabels(top_mem_3_sorted.index, fontsize=9)
        axes[1].set_xlabel('Memory (MB)', fontsize=10)
        axes[1].set_title('Most Memory Efficient', fontsize=11, fontweight='bold')
        for i, (idx, row) in enumerate(top_mem_3_sorted.iterrows()):
            axes[1].text(row['peak_memory_mb'] + 5, i, f"{row['peak_memory_mb']:.0f}MB",
                        va='center', fontsize=8)

        # Panel 3: Smallest (top 3)
        top_size_3 = self.get_best_models(results, criterion="size", top_k=3)
        top_size_3_sorted = top_size_3.sort_values('model_size_mb', ascending=False)
        colors3 = plt.cm.plasma_r(np.linspace(0.2, 0.8, len(top_size_3_sorted)))
        axes[2].barh(range(len(top_size_3_sorted)), top_size_3_sorted['model_size_mb'],
                     color=colors3, alpha=0.8)
        axes[2].set_yticks(range(len(top_size_3_sorted)))
        axes[2].set_yticklabels(top_size_3_sorted.index, fontsize=9)
        axes[2].set_xlabel('Size (MB)', fontsize=10)
        axes[2].set_title('Smallest Models', fontsize=11, fontweight='bold')
        for i, (idx, row) in enumerate(top_size_3_sorted.iterrows()):
            axes[2].text(row['model_size_mb'] + 2, i, f"{row['model_size_mb']:.0f}MB",
                        va='center', fontsize=8)

        plt.suptitle('Top 3 Performers Summary', fontsize=14, fontweight='bold', y=1.02)
        plt.tight_layout()
